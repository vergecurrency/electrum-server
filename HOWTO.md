How to run your own Verge Electrum server
===================================
...
you also need to run VERGEd and keep a copy of the full blockchain,
which is roughly 1.1 GB (September 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit VERGEd 
...
only leaving spare cycles for VERGEd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running VERGEd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `VERGEd` and Electrum.
...
### Step 2. Download and compile VERGEd

Please see https://github.com/vergecurrency/verge for detailed instructions.


### Step 3. Configure and start VERGEd

In order to allow Electrum to "talk" to `VERGEd`, we need to set up an RPC
username and password for `VERGEd`. We will then start `VERGEd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.VERGE
    $ $EDITOR ~/.VERGE/VERGE.conf

Write this in `VERGE.conf`:
...
If you have an existing installation of VERGEd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ VERGEd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `VERGEd`:

    $ VERGEd

Allow some time to pass for `VERGEd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ VERGEd getblockchaininfo

Before starting the Electrum server your VERGEd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start VERGEd at boot
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/vergecurrency/electrum-server.git
    $ cd electrum-server
...
https://github.com/vergecurrency/electrumdb


Alternatively, if you have the time and nerve, you can import the blockchain yourself.

As of September 2017, it takes about 2-3 days to import the blockchain manually, depending
...
1. To increase privacy of transactions going through your server
   you may want to close VERGEd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting VERGEd (together with electrum-server) on a weekly basis to clear out unconfirmed
...
irc.freenode.net #electrum-xvg and we'll try to congratulate you
on supporting the community by running a Verge Electrum node.

If you're operating a public Verge Electrum server please make a pull request
